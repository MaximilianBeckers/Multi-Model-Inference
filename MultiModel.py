...
class MultiModel:
    ...
    #***************************************
    def do_classification(self, num_classes, min_cluster_size, algorithm, reduced=False):
        ...
        if reduced:
            if algorithm == "kmeans":
                ...
                self.class_centers, self.class_labels = clustering.Ward(self.umap_embedding, num_classes);
            ...
            if algorithm == "kmeans":
                ...
                self.class_centers, self.class_labels = clustering.Ward(self.coord_array, num_classes);
            elif algorithm == "GaussianMixture":
        ...
        #take all classes with size bigger than min_cluster_size
        self.good_classes = np.argwhere(self.abs_class_size>=min_cluster_size).flatten();
        self.junk_classes = np.argwhere(self.abs_class_size<min_cluster_size).flatten();

        #set the labels for all junk classes to -1
        for tmp_junk_class in self.junk_classes:
            self.class_labels[self.class_labels == tmp_junk_class] = -1;
